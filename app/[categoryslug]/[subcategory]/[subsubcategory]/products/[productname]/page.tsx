...
async function getProductData(categorySlug: string, subCategorySlug: string, subSubCategorySlug: string, productSku: string) {
  try {
    ...
    // Récupère la sous-sous-catégorie
...
export default async function ProductPage({ params }: PageProps) {
  ...
  const { category, subCategory, subSubCategory, product } = data;
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-3">
              ...
              {product.isProductCategorySelected && (
                <div className="flex items-center justify-center text-purple-600">
                  <Star className="w-5 h-5 mr-2 fill-current" />
                  <span className="font-medium">Mis en avant dans la catégorie</span>
                </div>
              )}
            </div>
